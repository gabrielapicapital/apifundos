...
export function render(lista) {
  ...
  document.getElementById("countLabel").textContent = `${lista.length} fundo${lista.length === 1 ? "" : "s"} encontrados`;

  const { rentabilidadeMedia, positivos, negativos, pendentes } = resumo(lista);

  document.getElementById("summaryCards").innerHTML = `
    <div class="summary-card"><div class="label">Fundos na visão atual</div><div class="value">${lista.length}</div></div>
